...
def plot_histograms_comparing_two(results1, results2, output_filename):
    ...
    plt.subplot(1, 2, 1)
    #plt.hist(initial_ob_values1, bins=50, alpha=0.5, label='Initial OB - HILL KP', edgecolor='black', density=True)
    plt.hist(new_ob_values1, bins=50, alpha=0.5, label='New OB - HILL KP', edgecolor='black', density=True)
    #plt.hist(initial_ob_values2, bins=50, alpha=0.5, label='Initial OB - SA KP', edgecolor='black', linestyle='dashed', density=True)
    plt.hist(new_ob_values2, bins=50, alpha=0.5, label='New OB - SA KP', edgecolor='black', linestyle='dashed', density=True)
...
if __name__ == "__main__":
    ...
    for cities in range(20, 120, 20):
        #print(cities)
        for n in range(1, 5): 
            #print(n)    D:\CS-thesis-results\SA_KP_results
            items = n * cities
            #input_folder_problem_instances = f'problem_instances_ttp/json_files_TTP_instances_{cities}_items_{items}'
            result_file_TSP=f'D:/CS-thesis-results/hillclimber_KP_results/TTP_instances_{cities}_items_{items}/results_hillclimber_tsp_cities_{cities}_items_{items}.json'
            result_file_hybride=f'D:/CS-thesis-results/SA_KP_results/TTP_instances_{cities}_items_{items}/results_SA_tsp_cities_{cities}_items_{items}.json'
            output_filename=f'D:/CS-thesis-results/plots/visualization_hillclimber_SA_KP_comparison_cities_{cities}_items_{items}.png'
